...
class DiarizationEnhancedTask(Task):
    ...
    def run(self, dialog: Dialog, args: dict[str, Any] = {}) -> Dialog:
        ...
        segment_rows: list[tuple[float, str]] = []
        annotations_data = []
        ...
        for turn in dialog.turns:
            ...
            for seg_start, seg_end in segments:
                ...
                segment_rows.append((float(abs_start), rttm_line))
        ...
        # Backchannels: one RTTM segment per event (listener), using timeline abs times.
        for turn_index, turn in enumerate(dialog.turns):
            for bc_index, evt in enumerate(getattr(turn, "backchannel_events", []) or []):
                listener = evt.get("listener")
                if not listener:
                    continue
                abs_start = evt.get("abs_start")
                if abs_start is None:
                    continue
                dur = evt.get("clip_duration")
                if dur is None or float(dur) <= 0:
                    logging.warning(
                        "[DiarizationEnhancedTask] Skipping backchannel RTTM segment: missing or invalid "
                        f"clip_duration (turn_index={turn_index}, bc_index={bc_index}, "
                        f"host_speaker={turn.speaker!r}, listener={listener!r}, abs_start={abs_start}, "
                        f"audio_path={evt.get('audio_path')!r}). "
                        "clip_duration is normally set in AudioDialog.synthesize_backchannel_clips()."
                    )
                    continue
                dur = float(dur)

                if listener not in speaker_map:
                    speaker_map[listener] = f"SPEAKER_{speaker_count:02d}"
                    speaker_count += 1
                generic_listener_id = speaker_map[listener]
                abs_start_f = float(abs_start)
                abs_end = abs_start_f + dur
                rttm_line = (
                    f"SPEAKER {file_id} 1 {abs_start_f:.3f} "
                    f"{dur:.3f} <NA> <NA> {generic_listener_id} <NA> <NA>"
                )
                segment_rows.append((abs_start_f, rttm_line))
                annotations_data.append({
                    "start": abs_start_f,
                    "end": abs_end,
                    "speaker": generic_listener_id,
                    "type": "backchannel",
                })

        segment_rows.sort(key=lambda x: x[0])
        rttm_data = "\n".join(line for _, line in segment_rows)
